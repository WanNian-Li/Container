    ...
    while not np.all(treenow.nodes[id].data[0]==0):
        ...
        if np.any(nowbay==1):
            ...
            # 被阻挡的情况
                ...
                for tierblocking,keyblocking in enumerate(np.flipud(nowbay[x,y+1:])):   # tierblocking: 阻挡容器的索引值 keyblocking: 阻挡容器的值
                    ...
                    if keyblocking!=0:
                        ...
                        for putstack in range(stack):
                            if (putstack!=x) and (np.any(nowbay[putstack]==0)):
                                tiertarget=np.where(nowbay[putstack]==0)[0][0]  # tiertarget: 重定位的层索引值
                                ...
                                if LBlow(nowbay.flatten(), stack, tier)==0: # 存在最优方案时，直接将sig赋值为最优方案的标识符，在后续直接选择该方案
                                    # print(nowbay.reshape(stack, tier))
                                    sig=len(cckk)-1
                                # 复原 
                                nowbay[putstack,tiertarget]=0
                        # 一般情况
                        if sig==-1: #不存在最优方案，从所有候选方案中选取预测重定位次数最小的方案
